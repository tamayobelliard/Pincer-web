...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!dupRes.ok) {
      console.error('tables/create: dup check error', dupRes.status);
    ...
    if (!insertRes.ok) {
      ...
      if (insertRes.status === 409 || errText.includes('duplicate key')) {
        return res.status(409).json({ success: false, error: `Mesa ${table_number} ya existe` });
      }
      console.error('tables/create: insert error', insertRes.status, errText);
